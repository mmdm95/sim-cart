...
class CartsUtil implements ICartsUtil
{
    ...
    public function getItem(string $item_code, array $columns = ['pp.*']): array
    {
        ...
        $sql .= "ON {$this->db->quoteName('b')}.{$this->db->quoteName($brandColumns['id'])}={$this->db->quoteName('p')}.{$this->db->quoteName($productColumns['brand_id'])} ";
        $sql .= "WHERE {$this->db->quoteName('pp')}.{$this->db->quoteName($productPropertyColumns['code'])}=:__cart_item_code_ ";
